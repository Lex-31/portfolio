$(function () {
  ...
  $('.info-tabs__item').on('click', function (e) { //обрабатываем событие клика
    e.preventDefault();

    $('.info-tabs__item').removeClass('info-tabs__item--active'); //удалем подчеркивание ссылки
    $(this).addClass('info-tabs__item--active'); //добавляем подчеркивание текущей ссылки

    $('.info-tabs__content').removeClass('info-tabs__content--active'); //удаляем содержимое контента

    $($(this).attr('href')).addClass('info-tabs__content--active');  //берем значение атрибута href="" данной ссылки в виде #tab-1 - это ID html содержимого контента, контент добавляемся соответсвующий данной ссылке 
