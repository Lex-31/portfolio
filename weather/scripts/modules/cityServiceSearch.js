...
export const cityServiceSearch = (widget) => { //форма ввода города
  ...
  button.addEventListener('click', () => { //слушатель события клика по кнопочке
    ...
    inputCity.addEventListener('keyup', ({ code }) => { //когда открыто поле ввода города, нажимаем Escape - закрываем окно ввода
      if (code === 'Escape') { //если нажата кнопка Escape
        form.remove(); //удаляем элемент формы ввода города
      }
    });
  });
};